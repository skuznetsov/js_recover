...
function analyzeFileSize(filePath) {
    ...
    if (size >= THRESHOLDS.CRITICAL) {
        ...
        recommendations = [
            ...
            'Note: Node.js heap is already set to 16GB (--max-old-space-size=16386)',
            'If you have more RAM, edit app.js shebang to increase heap size',
            'Consider splitting the file if possible'
        ];
    } else if (size >= THRESHOLDS.DANGER) {
        level = 'danger';
        message = `⚠️  WARNING: File is ${sizeMB} MB - May run out of memory on systems with <16GB RAM`;
        recommendations = [
            'Use --no-grok to skip AI analysis (saves 40-60% memory)',
            'Use --no-unpack if not a webpack bundle (saves 30% memory)',
            'Node.js heap is set to 16GB (can edit app.js shebang if you have more RAM)',
