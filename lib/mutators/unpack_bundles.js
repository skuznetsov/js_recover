...
function saveWebpackModulesToFolder(modules, opts) {
    ...
    modules.forEach((mod, arrayIdx) => {
        ...
        try {
            // Generate code for this module with source maps
            const ast = t.program([
            ...
            // Generate code with source map support
            // Using @babel/generator default export for source map generation
            ...
            const sourceMapFile = `${moduleName}.map`;
            const code = header + result.code + `\n//# sourceMappingURL=${sourceMapFile}`;

            // Save the JavaScript file
            fs.writeFileSync(modulePath, code, 'utf8');

            // Generate and save source map
            const sourceMapPath = modulePath + '.map';
            const headerLines = header.split('\n').length - 1;

            // Since we strip location information during deobfuscation,
            // generate an identity source map (maps generated code to itself)
            // This still enables DevTools debugging features
            ...
            // Generate identity mappings: each line maps to itself
            codeLines.forEach((line, idx) => {
                if (line.trim().length > 0) {
                    // Map generated line to original (accounting for header)
                    generator.addMapping({
                        source: path.basename(inputFile),
                        original: { line: idx + 1, column: 0 },
                        generated: { line: idx + headerLines + 1, column: 0 }
                    });
                }
            });

            const sourceMapJson = generator.toString();
            const sourceMapObj = JSON.parse(sourceMapJson);

            // Add metadata for debugging
            sourceMapObj._note = "Identity source map: generated during webpack module extraction";
            sourceMapObj._originalBundle = path.basename(inputFile);
            sourceMapObj._moduleId = mod.id;

            fs.writeFileSync(sourceMapPath, JSON.stringify(sourceMapObj, null, 2), 'utf8');
...
`;
    ...
    const mappingData = {
        ...
        sourceMapGenerated: true,  // P1-3: Source maps enabled
        modules: moduleMap.map(m => ({
            id: m.id,
            filename: `${m.name}.js`,
            sourceMap: `${m.name}.js.map`,  // P1-3: Source map file
            patternName: m.name,
