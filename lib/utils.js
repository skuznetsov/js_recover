...
class Utils {
    ...
    static findVariableOnScope(node, varName) {
    ...
    static findFunctionOnScope(node, funcName) {
        let scope = this.findNodeScope(node);
        if (scope) {
            while(scope) {
                let variable = scope.functions[funcName];
                if (funcName in scope.functions) {
                    return scope.functions[funcName];
                }
                scope = scope.parent;
            }
        }
